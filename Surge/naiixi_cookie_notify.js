...
// @Surge
// @version      1.2.0
// @description  自动捕获 Naiixi 登录 Cookie 并消息通知输出完整格式
// ==/UserScript==
...
if (url.includes("naiixi.com") && $request.method === "GET") {
  ...
  if (cookie.includes("PassportId") && cookie.includes("ASP.NET_SessionId")) {
    const language = cookie.match(/language=([^;]+)/)?.[1] || "";
    ...
    const fullCookie = `language=${language}; tbName=${tbName}; tbPwd=${tbPwd}; ASP.NET_SessionId=${sessionId}; PassportId=${passportId}`;
    $persistentStore.write(fullCookie, "naiixi_cookie");

    $notification.post("✅ Naiixi Cookie 抓取成功", "已保存完整 Cookie", fullCookie);
